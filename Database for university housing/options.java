...
public class options extends javax.swing.JFrame{
	...
	private JTextField txtNearbySpotenterYes;
	...
	public options() {
		...
		mnViewCurrent.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0)
			{
				...
				dataBaseConnection();
				String str="SELECT INVOICES.INVOICEID,INVOICES.COST AS INVCOST,INVOICES.STUDENTID,INVOICES.STATUS,INVOICES.INVOICEDATE FROM INVOICES,STUDENT,SESSIONVARIABLES WHERE STUDENT.STUDENTID = SESSIONVARIABLES.ID_NUM AND INVOICES.STATUS = 'BILLED' AND INVOICES.INVOICEDATE IN (SELECT max(INVOICEDATE) from INVOICES)";
				try {
		...
		mnViewFormer.addMouseListener(new MouseAdapter() {
			...
			{
				String str="SELECT INVOICES.INVOICEID FROM INVOICES, STUDENT WHERE STUDENT.STUDENTID = SESSIONVARIABLES.ID_NUM PASS THE SESSION VARIABLE AND INVOICES.STATUS = 'BILLED'";
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			//dataBaseConnection();
			String str3="SELECT INVOICES.INVOICEID, INVOICES.COST AS INVCOST, INVOICES.STUDENTID, INVOICES.STATUS, INVOICES.INVOICEDATE FROM INVOICES WHERE '"+str2+"' = INVOICES.INVOICEID";
			try {
		...
		mnViewRequest.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				String str="SELECT TERMINATIONREQUEST.REQUESTID, TERMINATIONREQUEST.STATUS FROM TERMINATIONREQUEST, AGREEMENTS, SESSIONVARIABLES WHERE (TERMINATIONREQUEST.LEASEID_FORTERMINATION = AGREEMENTS.LEASEID AND AGREEMENTS.STDLEASEID = SESSIONVARIABLES.ID_NUM) UNION SELECT AG1.LEASEID, AG1.STATUS FROM AGREEMENTS AG1, SESSIONVARIABLES SV1 WHERE (AG1.STDLEASEID = SV1.ID_NUM AND (AG1.STATUS = 'PENDING' OR AG1.STATUS = 'INPROGRESS' OR AG1.STATUS =  'WAITING'))";
			try {
				rs=stmt.executeQuery(str);
				while(rs.next())
				{
		...
		mnCancelRequest.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try 
				{
					...
					while(rs.next())
					{
						String s1=rs.getString("LEASEID");
						String s2=rs.getString("TERMINATIONID");
		...
		txtNearbySpotenterYes = new JTextField();
		txtNearbySpotenterYes.setText("nearby spot(enter yes or no)");
		mnRequestNew.add(txtNearbySpotenterYes);
		txtNearbySpotenterYes.setColumns(10);
...
String str1="DECLARE CNT NUMBER; BEGIN SELECT COUNT(*) INTO CNT FROM AGREEMENTS, SESSIONVARIABLES WHERE SESSIONVARIABLES.ID_NUM = AGREEMENTS.STDLEASEID AND AGREEMENTS.STATUS = 'INPROGRESS'; IF( CNT > 0 ) THEN INSERT INTO PARKINGREQUEST (PARKINGREQID, PERSONID, REQUESTTIME, CLASSIFICATION, HANDICAPPED, NEARBYSPOT, STATUS) VALUES ((select COUNTERS.INDEXVALUE from COUNTERS where INDEXNAME = 'PARKINGREQID'), (select SESSIONVARIABLES.ID_NUM from SESSIONVARIABLES),sysdate, '"+s1+"', '"+s2+"', '"+s3+"', 'PENDING'); END IF; END;";
		...
		listModel1.addElement("water");
		listModel1.addElement("electricity");
		listModel1.addElement("appliances");
		listModel1.addElement("internet");
		listModel1.addElement("cleaning");
		listModel1.addElement("misc");
		
		txtDescription = new JTextField();
		txtDescription.setText("description");
		mnNewTicket.add(txtDescription);
		txtDescription.setColumns(10);
		
		JButton btnSubmit_19 = new JButton("submit");
		btnSubmit_19.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String s1=txtDescription.getText();
				String s2= list_1.getSelectedValue().toString();
				System.out.println(s1+" "+s2);
String str1="DECLARE CNT NUMBER, PLACENUMBER1 NUMBER; BEGIN SELECT COUNT(*) INTO CNT FROM RESIDENCE_HALL_ROOM  WHERE RESIDENCE_HALL_ROOM.PLACENUM IN ( SELECT  AGREEMENTS.PLACENUMBER INTO PLACENUMBER1 FROM AGREEMENTS,  SESSIONVARIABLES WHERE SESSIONVARIABLES.ID_NUM = AGREEMENTS.STDLEASEID AND AGREEMENTS.STATUS = 'INPROGRESS')  IF (CNT > 0) THEN INSERT INTO MAINTENANCETICKETS ( DESCRIPTION, TICKETTYPE,  STUDENTID,TICKETID, TICKETDATE) VALUES ( VFFE, VFEE, (SELECT  PERSON.PERSONID FROM PERSON, SESSIONVARIABLES WHERE  SESSIONVARIABLES.ID_NUM = PERSON.PERSONID), (SELECT COUNTERS.VALUE  FROM COUNTERS WHERE COUNTERS.INDEXNAME = 'MAINTENANCETICKET'), sysdate); END IF;  END;";
				try {
					rs=stmt.executeQuery(str1);
					if(rs.next())
					{
		...
		mnUpdateProfile.add(mnFamily);
		
		txtName = new JTextField();
		txtName.setText("first name");
		mnFamily.add(txtName);
		txtName.setColumns(10);
		
		txtLastName = new JTextField();
		txtLastName.setText("last name");
		mnFamily.add(txtLastName);
		...
		JButton btnSubmit_18 = new JButton("submit");
		mnFamily.add(btnSubmit_18);
