...
var loss=0;
    function drawScene() {
        ...
        {
win=1;
document.getElementById('uwon1').innerHTML='You won, your score is: '+score;
t1=1;
...
audio.play();
        ...
        if(current!=-2&&loss!=1)
        document.getElementById('uwon').innerHTML='Score: '+score;
    else{
        if(score!=0)
        document.getElementById('uwon').innerHTML='Score: '+(score-10);
        else
            document.getElementById('uwon').innerHTML='Score: '+score;
        }
        count1=-1;
...
//mat4.translate(mvMatrix,[transarr[0][0],transarr[0][1],transarr[0][2]]);
   ...
   //     mvPushMatrix();
        mat4.rotate(mvMatrix, degToRad(rcube6), [1, 1, 1]);
      //  mat4.rotate(mvMatrix, degToRad(rCube1), [0, 1, 0]);
      //  mat4.rotate(mvMatrix, degToRad(rCube2), [0, 0, 1]);
        gl.bindBuffer(gl.ARRAY_BUFFER, cubeVertexPositionBuffer);
        gl.vertexAttribPointer(shaderProgram.vertexPositionAttribute, cubeVertexPositionBuffer.itemSize, gl.FLOAT, false, 0, 0);
...
{
if(score!=0)
document.getElementById('uwon1').innerHTML='You lost, your score is: '+(score-10);
else
document.getElementById('uwon1').innerHTML='You lost, your score is: '+(score);
   ...
   //     mvPushMatrix();
     ...
     var tem=Math.floor(Math.random()*4);
       //     console.log(tem);
           ...
           if(ecurrent1==6 && tem==2)
                {
                  
                    tem=3;
                }
            while(pos_arr[ecurrent1][tem]==-1)
            {
                //alert(ecurrent1+"   "+tem);
                tem=Math.floor(Math.random()*4);
...
/*       var tem=Math.floor(Math.random()*4);
       //     console.log(tem);
           ...
           /* if(pos_arr[ecurrent][tem]==-1){
            ...
            if(ecurrent2==6 && tem==2)
                {
                  
                    tem=3;
                }
            while(pos_arr[ecurrent2][tem]==-1)
            {
                var tem=Math.floor(Math.random()*4);
            }


            earray2=coord_array[pos_arr[ecurrent2][tem]-1];
            dcount2=0;
            ecurrent2=pos_arr[ecurrent2][tem]-1;
        //    console.log(ecurrent);
        }
         gl.viewport(0, 0, gl.viewportWidth, gl.viewportHeight);
  //      gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
